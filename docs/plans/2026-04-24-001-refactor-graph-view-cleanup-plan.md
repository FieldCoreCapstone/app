...
- **Chart.js usage:** `static/js/main.js:315-317` uses `historyChart.destroy()` + `new Chart(...)` on every render. No existing `.update()` call. The 3s-tick refresh path must introduce `historyChart.update('none')` to avoid flashing; destroy+rebuild is reserved for metric/range changes where axis config changes.
